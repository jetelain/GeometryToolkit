...
namespace GameRealisticMap.Geometries
{
    public static class GeometryHelper
    {
        ...
        public static IEnumerable<Polygon> ToPolygon(Geometry geometry, Func<IEnumerable<Coordinate>, IEnumerable<Coordinate>> transform)
        {
            ...
            {
                return ((MultiPolygon)geometry).Geometries.SelectMany(p => ToPolygon((Geometry)p, transform));
            }
            if (geometry.OgcGeometryType == OgcGeometryType.Polygon)
            {
                ...
                {
                    new Polygon(
                        ToLinearRing((LineString)poly.ExteriorRing, transform),
                        poly.InteriorRings.Select(h => ToLinearRing((LineString)h, transform)).ToArray())
                };
            }
