...
namespace Pmad.Geometry
{
    ...
    public struct Matrix2x2FN : IMatrix2x2<float, Vector2FN, Matrix2x2FN>
    {
        public readonly Vector2FN X;
        public readonly Vector2FN Y;
        ...
        public Matrix2x2FN(Vector2FN x, Vector2FN y)
        {
            X = x; 
            Y = y;
        }

        public static Matrix2x2FN CreateRotation(float radians)
        {
            //var cos = MathF.Cos(radians);
            //var sin = MathF.Sin(radians); 
            (var sin, var cos) = MatrixHelper.SinCos(radians);
            return new Matrix2x2FN(new(cos, sin), new(-sin, cos));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector2FN Transform(Vector2FN value)
        {
            return (X * value.X) + (Y * value.Y);
        }

        public bool Equals(Matrix2x2FN other)
        {
            return other.X.Equals(X) && other.Y.Equals(Y);
        }
    ...
    public struct Matrix3x2FN : IMatrix3x2<float, Vector2FN, Matrix3x2FN>
    {
        public readonly Matrix2x2FN XY;

        public readonly Vector2FN Z;
        ...
        public Matrix3x2FN(Matrix2x2FN xy, Vector2FN z)
        {
            XY = xy; 
            Z = z;
        }

        public static Matrix3x2FN CreateRotation(float radians, Vector2FN centerPoint)
        {
            //var cos = MathF.Cos(radians);
            //var sin = MathF.Sin(radians);
            (var sin, var cos) = MatrixHelper.SinCos(radians);

            var z = new Matrix2x2FN(new (1 - cos, sin),new(-sin, 1 - cos))
                .Transform(centerPoint);
        ...
        public bool Equals(Matrix3x2FN other)
        {
            return other.XY.Equals(XY) && other.Z.Equals(Z);
        }
